...
class FileSystemTest extends TestCase
{
    ...
    public function testRecursiveDelete(): void
    {
        $rootPath = 'root';
        $rootItems = [
            'root' . DIRECTORY_SEPARATOR . 'folder1',
            'root' . DIRECTORY_SEPARATOR . 'file1',
        ];
        $folder1Items = [
            'root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'subFolder1',
            'root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'file11',
        ];
        $subFolder1Items = [];

        $this->mock->shouldReceive('scanFolder')->withArgs([$rootPath])->once()->andReturn($rootItems);
        $this->mock->shouldReceive('scanFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1'])
            ->once()->andReturn($folder1Items);
        $this->mock->shouldReceive('scanFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'subFolder1'])
            ->once()->andReturn($subFolder1Items);

        $this->mock->shouldReceive('isFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1'])
            ->once()->andReturn(true);
        $this->mock->shouldReceive('isFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'subFolder1'])
            ->once()->andReturn(true);
        $this->mock->shouldReceive('isFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'file1'])
            ->once()->andReturn(false);
        $this->mock->shouldReceive('isFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'file11'])
            ->once()->andReturn(false);

        $this->mock->shouldReceive('deleteFolder')
            ->withArgs(['root' . DIRECTORY_SEPARATOR . 'folder1' . DIRECTORY_SEPARATOR . 'subFolder1'])
            ->once()->andReturn(true);
        ...
        $this->mock->shouldReceive('delete')
            ...
            ->once()->andReturn(true);
        $this->mock->shouldReceive('deleteFolder')
            ->withArgs(['root'])->once()->andReturn(true);
